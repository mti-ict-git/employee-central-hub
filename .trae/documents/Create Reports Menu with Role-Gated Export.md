...
* Map to frontend models:
  ...
  * role\_permissions → per-role module actions: `{ module: 'employees'|'users'|'reports', action: 'read'|'create'|'update'|'delete'|'manage_users'|'export', allowed: boolean }`

  * role\_column\_access → per-role field access: `{ section: 'core'|'contact'|'employment'|'bank'|'insurance'|'onboard'|'travel'|'checklist'|'notes', column: string, read: boolean, write: boolean }`
...
* Expect backend endpoints (to be added server-side) that read existing tables:
  ...
  * `GET /api/rbac/permissions` → role\_permissions

  * `GET /api/rbac/columns` → role\_column\_access
...
* Validate sections hidden/disabled per `role_column_access` for hr\_general and finance as primary test cases.
